...
* if there is a field of the Configuration file which is empty ( you don't need this field) , you don't put this field in the configuration file , otherwise it will be an error . 

Organization of the configuration file:
---------------------------------------
The first level fields of the configuration file are the following elements :
...
they must always not be empty , if not it will be an error .
...
Each element of this array is a **package-name field** will contain , all the necessary fields to build the package , for example: Type field, Files field ,Build field ... ect
...
```
This field must exists and be not empty , in order to build packages , if not it will be an error .
It's contains the following elements :
...
**Type** : type of package you want to build , it must be one of the following types :  **binary** , **source** , **arch_independant** , **library** , if not it will be an error , and if this field is empty it will be another error (this field must always exists).

**Files** : this field will contain an array of files of the packages , each element of  this array is a file  of the package .
Each  element of this array represent **one file of the package** contain at least the **source** and the **destination** , is presented in two different ways:
...
If there is nothing need before building the package , you don't put this field on the configuration file , because if we add empty field , it will be an error .
...
It's an array of dependencies , each element of this array is the name of the dependence,and each **dependence field**  can be represent in different ways:

* if the name of dependence is the same for all distributions ( versions and architectures) the content of **dependence field** will be **"*"** , for example :
...
It's an array containing one element which is the **Dependencies field** .
The description of this field is the same with the **Dependencies field of Build field** (see above) .

**example:**
you have tcc (runtime dependence) which is necessary for Archlinux and Centos (but not for Debian), and for Archlinux the name of this dependence is different (tcc-1.2)
...
**Install** : this field contrain pre-install and post-install commands , it's an array of this two elements , and each elements is a list of commands to execute , if there is no pre-install and post-install commands , we don't write this field , because if we write it and we leave it empty , it will be an error . This field contains the following elements: 
